...
namespace ccfdns
{
  ...
  // minimal time EAT public keys must be discoverable before issuing tokens (1
  // day)
  ...
  // We could also keep the public keys in TLSA records, for DNS-based discovery
  // and transparency but then aDNS clients can already directly fetch the
  // service attestation records.
  ...
  struct EATPublicKeyRecord
  {
    ...
    // private-key erasure time + upper bound on the expiration of all signed
    // tokens
  ...
  const std::string eat_issuer_key_info_table_name =
    "eat_issuer_key_info_table_name";

  using EATPrivateKeys = ccf::ServiceValue<std::vector<ccf::crypto::Pem>>;
  const std::string eat_private_key_table_name =
    "private:eat_private_key_table_name";
