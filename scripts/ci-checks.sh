...
group "Python format"
if [ $FIX -ne 0 ]; then
  git ls-files tests/ python/ scripts/ tools/ demo/ demo/client demo/server demo/adns | grep -e '\.py$' | xargs black
else
  git ls-files tests/ python/ scripts/ tools/ demo/ demo/client demo/server demo/adns | grep -e '\.py$' | xargs black --check
fi
endgroup
